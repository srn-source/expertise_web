...
def app():
    ...
    # 1) By assigned internal validator in the table: TD_insert
    username = st.session_state['userName']
    
    if not username.lower().startswith("admin"):
        sql_query_by_instructions_assigned = """SELECT TOP 1 * from View_vistec_check WHERE  review_status = 'PASS' and vistec_chk IS NULL
    and assigned_internal_validator = '{}'""".format(username)  
    
        df_new = cursor.execute(sql_query_by_instructions_assigned)
        df_new = df_new.fetchall()
        if len(df_new) == 0:
            st.error("No instruction is assigned to you.")
    else:
        if "ADMIN3" in st.session_state['userName'].upper(): 
            df_new = cursor.execute("SELECT TOP 1 * from View_vistec_check WHERE  review_status = 'PASS' and vistec_chk IS NULL and (article_id like '%0' or article_id like '%4') ")
            df_new = df_new.fetchall()
            ...
            df_new = cursor.execute("SELECT TOP 1 * from View_vistec_check WHERE  review_status = 'PASS' and vistec_chk IS NULL and (article_id like '%7' or article_id like '%8' )")
            df_new = df_new.fetchall()
            ...
            df_new = df_new.fetchall()
        elif "ADMIN2" == st.session_state['userName'].upper(): 
            df_new = cursor.execute("SELECT TOP 1 * from View_vistec_check WHERE   review_status = 'PASS' and vistec_chk IS NULL and (article_id like '%6' )")
            df_new = df_new.fetchall()
    ...
    if len(df_new)  == 1 and ( "ADMIN6" not in st.session_state['userName'].upper() and "ADMIN10" not in st.session_state['userName'].upper() and "ADMIN11" not in st.session_state['userName'].upper() and "ADMIN12" not in st.session_state['userName'].upper() and "ADMIN13" not in st.session_state['userName'].upper() and "ADMIN14" not in st.session_state['userName'].upper() and "ADMIN15" not in st.session_state['userName'].upper() and "ADMIN16" not in st.session_state['userName'].upper() and "ADMIN17" not in st.session_state['userName'].upper() and "ADMIN18" not in st.session_state['userName'].upper() and "ADMIN19" not in st.session_state['userName'].upper()):
